...
SELECT COUNT( * ) AS facilities_no_charge
FROM Facilities
WHERE membercost =0
...
JOIN (SELECT bookings.facid, bookings.memid
	  FROM country_club.Bookings bookings
	  JOIN country_club.Facilities facilities
	  ON bookings.facid = facilities.facid
	  AND facilities.name LIKE '%Tennis Court%') sub
...
JOIN country_club.Bookings bookings ON facilities.facid = bookings.facid
GROUP BY 1 
